DEFAULT_USER=" 【卐乂Oo☆㊣↙煞氣a筆記本↗㊣☆oO乂卍 】─=≡Σ((( つ•̀ω•́)つLET’SGO!"
...
prompt_context() {
  if [ $DEFAULT_USER ]; then
    #prompt_segment black 222 "%(!.%{%F{yellow}%}.)$DEFAULT_USER"
    prompt_segment 53 12 "%(!.%{%F{yellow}%}.)$DEFAULT_USER"
  elif [[ "$USER" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    prompt_segment 5 4 "%(!.%{%F{yellow}%}.)$USER@%m"
  fi
}
...
prompt_git() {
  ...
  local ref dirty mode repo_path
   ...
   if [[ "$(command git rev-parse --is-inside-work-tree 2>/dev/null)" = "true" ]]; then
    ...
    zstyle ':vcs_info:*' unstagedstr '(σ′▽‵)σ'
